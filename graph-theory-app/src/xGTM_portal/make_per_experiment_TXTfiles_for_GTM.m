%% make_per_experiment_TXTfiles_for_GTM
...
%
function [outdir, std_space_in] = make_per_experiment_TXTfiles_for_GTM(sleuth_text_file, gtm_folder_path, filter_0none_1exp_2pap)
%% Chunk 1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf('My filter is %s\n', filter_0none_1exp_2pap);

if filter_0none_1exp_2pap == '1' %experiment filter
    if ~exist('per-experiment_TXTfiles-expFilter', 'dir'), mkdir('per-experiment_TXTfiles-expFilter'); end
    outdir = 'per-experiment_TXTfiles-expFilter';
elseif filter_0none_1exp_2pap == '2' %paper filter
    if ~exist('per-experiment_TXTfiles-papFilter', 'dir'), mkdir('per-experiment_TXTfiles-papFilter'); end
    outdir = 'per-experiment_TXTfiles-papFilter';
else %no filter
    if ~exist('per-experiment_TXTfiles-noFilter', 'dir'), mkdir('per-experiment_TXTfiles-noFilter'); end
    outdir = 'per-experiment_TXTfiles-noFilter';
end

fprintf('My outdir is %s\n', outdir);

theFILE = sleuth_text_file; 
txtpath = gtm_folder_path;
%cd(txtpath)
...
if filter_0none_1exp_2pap == '1'
    ...
    final_exp_filt_paper_global     = [];
    final_exp_filt_subject_num      = [];
    ...
    for temp_it = 1:length(suspicious_papers)                              % FOR loop through SUSPICIOUS PAPERS
        ...
        temp_num_subjects          = str2num(char(clean_out(temp_expfilt_indices,3)));
        temp_exp_index_local       = temp_expfilt_indices;
        ...
        for temp_it2 = 1:size(dupRowValues_expfilt,1)                      % FOR loop through duplicate coordinates
            ...
            % find MAXIMUM subject number for experiments that get combined
            temp_maxExp_for_combo = max(temp_num_subjects(temp_inds_for_dups_in_temp_sus_coord_cells));
            % set the subject amount for the first experiment sharing a
            % duplicate coordinate to the maximum subject amount across all
            % those duplicate experiments
            temp_num_subjects(temp_inds_for_dups_in_temp_sus_coord_cells(1)) = temp_maxExp_for_combo;
            % remove subject amounts for other experiments getting combined
            temp_num_subjects(temp_inds_for_dups_in_temp_sus_coord_cells(2:end)) = [];
            temp_exp_index_local(temp_inds_for_dups_in_temp_sus_coord_cells(2:end)) = [];
            % 
        ...
        final_exp_filt_paper_global     = [final_exp_filt_paper_global;temp_exp_index_local];
        final_exp_filt_subject_num      = [final_exp_filt_subject_num;temp_num_subjects];
    ...
    if isempty(index_for_non_sus_papers)
        index_for_non_sus_papers = 1;
    end
    ...
    for temp_it = 1:length(non_suspicious_papers)
        ...
        final_exp_filt_paper_global             = [final_exp_filt_paper_global; ...
                                                    non_sus_paper_cells{temp_it}];
        final_exp_filt_subject_num              = [final_exp_filt_subject_num; ...
                                                    str2num(char(clean_out(non_sus_paper_cells{temp_it},3)))];
    ...
    if strcmp(final_exp_filt_paper_labels(1),""); bmp1=1; else; bmp1=0; end
    for temp_it = 1:exp_filt_final_num_exps                                % FOR export all text files for experiments
        exp_filt_cur_pape_for_filename = final_exp_filt_paper_labels(temp_it+bmp1);
        ...
        exp_filt_file_temp         = [header_Tal;...
                                      [exp_filt_temp_exp_labellll "" ""];...
                                      [sprintf("// Subjects=%d",final_exp_filt_subject_num(temp_it)) "" ""];...
                                      string(exp_filt_temp_exp_coordsss)];
        exp_filt_file_temp_realnum = [header_Tal;...
                                      [exp_filt_temp_exp_labellll "" ""];...
                                      [sprintf("// Subjects=%d",final_exp_filt_subject_num(temp_it)) "" ""];...
                                      string(exp_filt_temp_exp_coordsss)];
        ...
        exp_filt_ALL_will_be_table = [exp_filt_ALL_will_be_table;exp_filt_file_temp_realnum(2:end,:);"" "" ""];
        ...
        if cur_pap_ind_exp_filt<10
            cur_pap_string = strcat("00000",num2str(cur_pap_ind_exp_filt-(1-bmp1)));
        elseif cur_pap_ind_exp_filt<100
            cur_pap_string = strcat("0000",num2str(cur_pap_ind_exp_filt-(1-bmp1)));
        elseif cur_pap_ind_exp_filt<1000
            cur_pap_string = strcat("000",num2str(cur_pap_ind_exp_filt-(1-bmp1)));
        elseif cur_pap_ind_exp_filt<10000
            cur_pap_string = strcat("00",num2str(cur_pap_ind_exp_filt-(1-bmp1)));
        elseif cur_pap_ind_exp_filt<100000
            cur_pap_string = strcat("0",num2str(cur_pap_ind_exp_filt-(1-bmp1)));
        else
            cur_pap_string = num2str(cur_pap_ind_exp_filt-(1-bmp1));
        end
...
%                                               string(temp_pap_filt_coords_noReps)]);
        cur_pap_table                 = table([strcat("// Reference=",reference_space) "" "";
                                              [strcat("// ",cur_pape,": All Paper-level Filtered Coordinates") "" ""];
                                              [sprintf("// Subjects=%d",max(str2num(char(clean_out(temp_exp_cell_indices,3))))) "" ""];
                                              string(temp_pap_filt_coords_noReps)]);
        if temp_it<10
            cur_ind_string = strcat("000",num2str(temp_it));
        elseif temp_it<100
        ...
        writetable(cur_pap_table,strcat('MA_',cur_ind_string,'_',cur_label,'_PaperFiltered.txt'),'Delimiter','\t','WriteVariableNames',false);
...
if filter_0none_1exp_2pap == '0'
    ...
    for i = 1:tot_exps
        bmap_cell_tabs{i} = table([strcat("// Reference=",reference_space) "" "";
                                  ...
                                  [sprintf("// Subjects=%s",clean_out(i,3)) "" ""];
        ...
        writetable(bmap_cell_tabs{i},strcat('MA_',cur_ind_string,'_',cur_label,'.txt'),'Delimiter','\t','WriteVariableNames',false);
...
currentFolder = pwd;
fprintf('My working dir is %s\n',currentFolder);
if ~exist(strcat(gtm_folder_path,'MA_Z'), 'dir'),  mkdir(strcat(gtm_folder_path,'MA_Z'));  end
